...
function do_sort(sort) {
	var searchForm = document.search;
	searchForm.common_sort.value = sort;
	searchForm.reQuery.value = "2";
	searchForm.submit();
}

//지점구분 선택
function sel_branch(cd) {
	var searchForm = document.search;
	searchForm.branchCd.value = cd;
	//searchForm.reQuery.value = "2";
	searchForm.submit();
}
...
function doSearch() {
	var searchForm = document.search; 
	var col = searchForm.collection.value;
	if (searchForm.query.value == "") {
		alert("검색어를 입력하세요");
		return;
	} else {
//		alert("2");
		searchForm.collection.value = col;
